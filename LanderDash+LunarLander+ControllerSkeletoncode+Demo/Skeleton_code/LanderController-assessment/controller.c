...
void *dashboard(void *data)
{
    ...
    while (true)
    {
        ...
        sendto(a, buffer, strlen(buffer), 0, daddr->ai_addr, daddr->ai_addrlen);
        a = recvfrom(d, buffer, bufsize, 0, NULL, NULL);
        buffer[a] = '\0';
...
void *datalogging(void *data)
{
    ...
    struct logging input1 = {200};
    struct logging input2 = {85};

    // write struct to file
    fwrite(&input1, sizeof(struct logging), 1, outfile);
    fwrite(&input2, sizeof(struct logging), 1, outfile);
